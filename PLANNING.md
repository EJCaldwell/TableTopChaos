...
- [x] 8.1 — Test infrastructure + unit/component tests (Vitest + RTL) *(**129 tests, 6 files, all passing.** Vitest 3 + jsdom + RTL + v8 coverage; `npm test` / `test:watch` / `coverage`. 100% statements on dice, HP/death-saves, safe-markdown and username rules — three of those EXTRACTED from components to be testable, with dice randomness injected so totals are asserted exactly. Component test for the campaign roster **closes 7.4's rendering gap**: `username (Character)`, identical for player and DM, no "Unnamed adventurer", survives a failed character lookup. Tests are type-checked by `tsc -b`, which caught two unsound casts Vitest ran green. Whole-app coverage 2.48%, reported honestly. **Not done:** `extractNpcHp`, initiative sort, import id-remap; every other panel. **Extended 2026-09-01** after the owner asked whether tests had been added per phase — they had not: Vitest only arrived here in 8.1, and Phases 1–7 were built with no runner. A targeted scan (not a back-fill sweep) found four modules whose PURE logic was untested and could plausibly break, and each was extracted into its own module and tested: `auth/authErrors` (11), `billing/state` (9), `schedule/when` (12), `media.fittedSize` (8). Deliberately NOT tested: panels that render a list over a Supabase call, where RLS and the 123-assertion matrix are the real test. **215 tests total.** The going-forward rule is now in CLAUDE.md: add tests with the work, do not back-fill old code speculatively)*
...
- [~] 9.2 — Vision toggle & obstructions (walls + freehand) *(COMPLETE 2026-09-02, then reopened the same day for **0066**: walls the players can see, opt-in per wall. A deliberate hole in 0061 — default FALSE, so an untouched campaign keeps 0061's privacy exactly, and 0061's own assertion was UPDATED rather than deleted (two select policies now, any member-facing one must be gated on `visible_to_players`; a third fails the deploy). Not every wall is a secret: a chasm edge or a portcullis is scenery the party can already see in the picture. Matrix **141**. Visible walls draw **above the fog** but **clipped to line of sight** — the two owner decisions sound contradictory and are not: a wall is not a secret you forget, but you only see the stretch of it you can actually look at. Under the fog it vanished behind you; unclipped it drew a wall's full extent from one glimpse. The clip reuses the tokens' path, so a wall and the creature against it cannot disagree about where the light stops. The area's stacking order (grid → tokens → fog → walls) is now written into the component, having been wrong twice in ways that each looked like a different bug. The DM's control defaults to VISIBLE while the column defaults to hidden — deliberately opposite: data written without an explicit choice should be secret, but the wall a DM draws by hand is usually the room the party is standing in. G4–G9 browser steps NOT RUN)*
  ...
  - [x] 9.2.2 — Web UI *(BUILT 2026-09-01, closed 2026-09-02 once the vision toggle had a visible half to test: `walls.ts` (pure geometry, **23 tests**), `WallLayer` (SVG overlay, line/room/freehand/erase, dashed live preview), wall tools + Clear in the DM strip, vision toggle in the Maps tab. Walls render LAST so they sit above tokens, and are click-through unless a tool is armed; arming one suspends token dragging entirely. **Two defects found by the tests before any UI existed**: an infinite loop when the simplifier's tolerance was 0, and a 2.8s freeze on a 12000-point stroke — both invisible to a browser checklist. **Foundation browser subset RUN and PASSED 2026-09-01** — including the mode collision (a tool armed must not drag a token) and, critically, that a player's window renders no walls at all. The rest is deferred to 9.3, which will exercise it anyway — QA/9.2_tests/)*
  - [x] 9.2.3 — QA *(COMPLETE 2026-09-02 — matrix **134/134**, 297 tests, `qa:checks` 62/62. The **foundation browser subset was run by the owner 2026-09-01 and PASSED** — drawing, erasing, persistence, the mode collision, two-window sync, and the player seeing no walls. Section C (vision toggle) and four cosmetic steps (rooms, freehand, zoom, clear-walls) were deliberately deferred to 9.3 rather than passed — the toggle stored a boolean and did nothing visible until fog existed. **This note used to claim they "were run and PASSED there on 2026-09-01/02"; that is not supported at step level** and was corrected 2026-09-02 during the checkbox back-fill. 9.3's run logs record passes for 9.3's OWN sections, which cover fog and sight rather than those five. Probably exercised in passing while building maps to test fog against — but probably-exercised is not a result, so they stay unticked)*
- [~] 9.3 — Token-based line of sight & sight range *(COMPLETE 2026-09-02 — server-side vision, fog, wall collision, diagonal movement; migrations 0062–0065 — then reopened the same day: own-move vision recomputes now wait **0ms** rather than 250. **The real cost was elsewhere**: the functions service's `SUPABASE_URL` is the PUBLIC gateway hostname, so every query the vision function made left Railway and came back in through the edge, from a container milliseconds from the database. `SUPABASE_INTERNAL_URL` fixes that for every Edge Function; the vision function's four sequential queries also became two parallel pairs. **The floor is two sequential round trips** — the move must be written before vision is recomputed, since the server computes from the stored position — so the work was making each trip cheap, not overlapping them. Cold starts (~500ms–1.1s against ~100ms warm) are now the biggest variable, so a player on a fogged map sends a 60s heartbeat that doubles as a backstop for a missed realtime event. **G/H steps RUN and PASSED 2026-09-02.** Then: the DM can select any token and see ITS line of sight drawn on the map — computed in the browser, since a DM already holds every wall, so 0061's server-side requirement never applied to them. An inspection overlay (lights up what is seen) rather than fog (darkens what is not), because fogging the DM's own view would hide the board they are looking at. Also confirmed WITHOUT changing anything: the DM could already move tokens through walls — the 0063 trigger exempts them and the client clamp is null for them — recorded because "I built that" and "that already worked" are different facts. Then **0067** — `blocks_movement`, so a wall can block sight without stopping anyone (a curtain, a hedge, fog); a column rather than a new `kind`, since the sight/movement pair wants two independent booleans and the mirror case (a chasm you see over but cannot cross) is the other combination. Sight-only walls draw dashed for the DM. **And a regression fixed**: the wall-clipping wrapper added an hour earlier covered the whole map above the tokens with `pointer-events: auto`, so the DM could not select ANY token — it presented as "I can't click tokens", not as a wall bug. Matrix **143**. **Remembered walls were built and then REMOVED the same day.** The design was sound — cell rasterisation bounded by map size rather than session length, horizontal run merging, half-cell padding for walls drawn on cell boundaries, corner sampling so a wall at the limit of sight counts as seen — but after the boundary fixes, exploration was generous enough that a few moves marked most of the map, so "is it remembering correctly?" and "is it showing everything?" looked identical from the map. A feature you cannot tell is working is not finished. `explored.ts` and its 20 tests are deleted rather than left unused; the design is recorded in the 9.3 QA log if persistent fog-of-war is ever wanted, which is where it belongs. **Visible walls are now simply always drawn in full** — three models were tried (under the fog: vanished when you looked away; clipped to sight: needed memory; clipped to memory: unobservable) and this is the only one never wrong. Nothing is leaked, because RLS sends a player only the walls marked visible (0066) — there was never anything to clip for safety, only for atmosphere. Then a batch of six owner reports 2026-09-02, of which two were real defects rather than requests. **`movePolygons` was swept against EVERY wall**, so the client refused to walk through the very sight-only walls 0067 exists to allow — the database permitted the move and the client never sent it, which is precisely why the server-side matrix could not see it and why "the client check is only a convenience" has to stay true in both directions. **Token hit areas were square while tokens are round**: the corners are ~21% of the element, empty map that answered clicks, and on a 4x4 monster they reach into neighbouring squares — `clipPath: circle(50%)` clips hit-testing as well as painting. Tokens now also paint biggest-first so the small, hard-to-hit token sits above the large one. A **5px drag threshold** stops a click near a token's edge nudging it a square. **Occupancy (0068)**: two tokens may not share space, enforced by a trigger against FOOTPRINTS rather than centres, with the DM deliberately NOT exempt — a wall is a fiction the DM authors and may cross, two creatures in one square is a mistake. And the fog latency the owner spotted between the two windows was never computation: the DM computes locally from walls they hold, while a player must wait for a WRITE and then a recompute, two sequential round trips by construction. The vision function now accepts a speculative `at` viewpoint, so the client asks BEFORE the move lands; it grants nothing, since the sweep is still bounded by the real walls and the token's real sight range. Walls are polled twice a second (owner's figure) because realtime cannot carry them — a player has no read on the table by design, and subscribing them would undo 0061. Then **neighbourhood prefetch**, from the owner's question about preloading everything: most of it already was — the map, the grid and every token row are all on the client, and fog is painted over them. The walls are the one thing that cannot be, which is 0061 and the reason the Edge Function exists at all. So the SERVER precomputes the ring instead: it holds the walls and a sweep is the cheap part, so answering for the eight surrounding squares costs one request instead of nine, and a single step now paints from memory in the same frame with no network involved. The leak is bounded and recorded — a browser knows what it would see one step away, slightly before stepping, which is learned by stepping anyway. The cache is REPLACED by every response and never merged, because a growing one would eventually light a doorway the DM had erased, with the failure appearing a step after the change that caused it. Degrades silently with more than one own token (no single square to ring) or past 400 wall segments (seventeen sweeps rather than two). Then **deselection**, which turned out never to have been built: `setSelectedId` was only ever called with a token id, so once anything was selected it stayed selected and switching tokens meant hitting the new one exactly. Step A6 of the earlier checklist asserts that behaviour and I had written it assuming it worked — an unrun step is a claim, not evidence. The fix needed a second change to be visible at all: the token clipping wrapper is `inset: 0` with default hit-testing, so it was the target of every click that was not on a token and the background never was. **That is the same wrapper that broke token selection a day earlier** — twice the symptom was about clicking and twice the cause was a transparent layer. Now `pointer-events: none` with the tokens setting `auto` back on themselves. **And the handler still never ran**: its guard read `if (wallTool)`, but `wallTool` is a string sentinel whose idle value is `'none'` — truthy — so it bailed out of every click ever made. It type-checked perfectly, since a string is a valid condition, and every other use of `wallTool` in the file compares against `'none'` explicitly. A value whose "off" state is truthy will eventually be tested for truthiness; `WallTool | null` would have made it unwriteable. Then **jumpy light**, which was ORDERING and not timing: overlapping requests (re-centre, 500ms wall poll, heartbeat, commit refresh) had no ordering guarantee, so an older response landing last snapped the light backwards and the next one snapped it forward — requests now carry a sequence number and stale answers are dropped. Compounded by the stored position LAGGING the visible one: a keyboard walk writes 300ms after the last step, so any refresh without an explicit `at` was computed from where the walk began. Every refresh now anchors to the current on-screen position, which also closes a fault the prefetch introduced — the cache was keyed by the anchor while the server swept from the stored position, filing the wrong light under the right square. Re-centre tightened 300→150ms because the ring is one square deep, so every square in it is its edge. **Stated plainly: a held walk re-centres close to once per step, so the prefetch buys an instant PAINT rather than fewer requests** — the right way round, since the player waits on the paint, but not the saving the name implies. Matrix **154**, 313 tests. A1–A10, C1–C3, D1–D8, G1–G5 **the whole outstanding browser backlog is now ONE file** — QA/9.3_tests/consolidated-run.md — **run by the owner 2026-09-02: 38 of 39 PASS**. The one failure was **3.3, "the 4x4 acts as a 5x5 hitbox"** — and the occupancy maths was exact; the token was in the wrong PLACE. Which lattice a centre belongs on depends on its size (odd/half on cell centres, even on corners), and the Size control wrote `size_cells` alone, so a 1x1 promoted to 4x4 stayed on the centre lattice, half a cell out, straddling five columns. `resnapTokens` already did this for a GRID change; resizing is the same event from the other side and was the one path with no re-snap. Fixed by writing x/y/size_cells together; 3 tests written as the failure, incl. the shrink direction. **316 tests.** **That was a real fix for a different problem: 3.3 came back.** The live data settled it — the 4x4 was already exactly on the corner lattice. **The owner's map has grid size 85, an odd number, and that is the bug.** Coordinates are integers (0048) but a cell centre on an odd grid is a half pixel, so it is stored rounded, moving each edge up to 0.5px — and two tokens round independently, so the gap can be a full pixel out against a **0.25px** tolerance. Rounding always goes one way, so the right and bottom pushed the neighbour AWAY while the left and top pushed TOWARD: a symmetric cause with a one-sided symptom, which is why it read as "the token is too big". Invisible on an even grid — every test, probe and matrix fixture uses 70. Tolerance now **1.5px** (**migration 0069** + `grid.ts`), 4 tests on grid 85. Also: the selection indicator the owner asked for **already existed** and I had broken it an hour earlier — `clip-path: circle(50%)`, added to fix hit areas, clips everything drawn outside the border box, so it deleted the outline AND the drop shadow under ringless tokens. Hit-testing moved into the pointer handler, where it costs no visuals. Matrix **154/154**, **320 tests**. L1–L7 re-check NOT RUN. It collapses nine stacked checklists (82 steps): 23 WITHDRAWN because they test walls-clipped-to-sight and remembered walls, both built then deliberately removed, so running them would fail correctly; ~25 were duplicates written over two days against a moving target ("fog follows your own move" was C1, H11 and I1). fog-refinements.md stays as the dated history, boxes unticked; B1–B4, E1–E8 and F1–F5 WITHDRAWN as they test removed behaviour)*
  - [x] 9.3.1 — Backend *(migration **0062**: `sight_squares` (NULL = unlimited, 0 = blind) + `dark_sight_squares`, in GRID SQUARES so they survive a re-grid — 0056's reasoning. Sight joined the DM-only guard, which is the sharpest case of it: a player able to widen their own sight could see the whole map. Matrix **136**)*
  - [x] 9.3.2 — Web UI *(BUILT 2026-09-01, refined through 2026-09-02: `vision.ts` (angular-sweep visibility polygon, **25 tests**), the **`vision` Edge Function** (computes where the walls are, returns polygons ONLY), `FogLayer` (SVG mask — white fogs, black polygons punch holes, so multiple tokens union for free and no polygon boolean maths exists anywhere). Fog renders LAST so it hides tokens as well as floor. Fails CLOSED: a failed request fogs everything rather than revealing the map. **The tested maths and the deployed maths are the same code** — `_geometry.ts` is generated by `npm run sync:geometry` and a test fails if it drifts, proven by tampering. **Owner QA found two problems, both fixed 2026-09-01.** (1) Fog at 92% opacity showed every token behind a wall as a faint disc — partial fog hides the floor partially and gives away everything standing on it. Now fully opaque, AND tokens outside the visible area are not rendered at all, since an invisible token is still in the DOM. (2) Walls now block movement, as **0063** + **0064** — server-side of necessity, because the client has no walls to check against. Then four more owner requests: DM-set fog density (**0065**), tokens clipped to the lit area so a half-seen creature shows only its visible part, moves REFUSED at a wall rather than reverted (via a second `movePolygons` polygon — walls-only, so the client can stop a drag without being told where the wall is), and grid-snapped Line/Room walls (to intersections, not cell centres). **Plus a real bug found by the owner**: token artwork vanished because the style set the `background` SHORTHAND after `backgroundImage`, silently resetting it — no error, and the storage logs showed the image being fetched successfully. Extracted to `tokenStyle.ts` with a test that the shorthand is never emitted, since the failure is a property of key ORDER that no build or type can catch. **Owner-run QA of all of that PASSED 2026-09-01.** Then **diagonal movement** (2026-09-02): keyboard movement was orthogonal only, so `movementDelta` now maps arrows, numpad 1–9 AND Home/PgUp/End/PgDn — the last set because most laptops have no numpad and would otherwise have no diagonal at all. Numpad is read from `event.code`, never `event.key`, since `key` is '7' or 'Home' depending on NumLock; a test asserts both paths agree. 7 tests. Then **two-arrow diagonals** the same day, after the owner said they could not test a full-size keyboard at all — holding Left+Up is the input everyone knows from games and needs no keys beyond the four arrows, making the numpad path the fallback rather than the feature. `combinedDelta` sums held keys and clamps to one cell per axis; held keys are cleared on blur, or a token that loses focus mid-press keeps a key "held" forever and every later press moves diagonally. 8 more tests. Then a **70ms grace window** on the first step of a press, because two keys are never pressed on the same millisecond and acting on the first alone produced "left, then up-left"; only the first step waits, so holding a direction still walks at full cadence. Its own risk is a fast tap being swallowed by the pending timer, so a keyup with a step pending takes that step immediately. **F1–F9 RUN and PASSED 2026-09-02**, including the two that pull against each other: keys pressed a fraction apart giving one diagonal step, and a very fast tap not being swallowed by the pending timer)*
  - [x] 9.3.3 — QA *(COMPLETE 2026-09-02 — matrix **134/134**, 297 tests, `vision` live. The owner's first run found the fog leak and asked for wall collision; both are built and the run needs repeating. **Two process lessons recorded in the run log**: a badly-written assertion turned out to be a working exploit (the two-step wall crossing), and a careless splice deleted five assertions while the matrix still reported "all passed" — assertions that do not exist cannot fail, and the only signal was the total dropping. That is why the count is quoted here on every change)*
