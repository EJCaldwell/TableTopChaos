...
- [~] 9.2 — Vision toggle & obstructions (walls + freehand) *(COMPLETE 2026-09-02, then reopened the same day for **0066**: walls the players can see, opt-in per wall. A deliberate hole in 0061 — default FALSE, so an untouched campaign keeps 0061's privacy exactly, and 0061's own assertion was UPDATED rather than deleted (two select policies now, any member-facing one must be gated on `visible_to_players`; a third fails the deploy). Not every wall is a secret: a chasm edge or a portcullis is scenery the party can already see in the picture. Matrix **141**. Visible walls draw **above the fog** — a wall the party can see is a landmark they know about, and you do not forget a cliff is there because you walked away from it. The area's stacking order (grid → tokens → fog → walls) is now written into the component, having been wrong twice in ways that each looked like a different bug. The DM's control defaults to VISIBLE while the column defaults to hidden — deliberately opposite: data written without an explicit choice should be secret, but the wall a DM draws by hand is usually the room the party is standing in. G4–G9 browser steps NOT RUN)*
