...
- [x] 1. Open "Main Test". The **campaign name is centred in the top app bar**,
      with a small **DM** badge beside it. There is **no separate campaign title
      bar** below it, and **no "Switch to" dropdown anywhere**.
- [x] 2. The workspace **fills the browser window** — app header, then the rail
      and workspace area down to the bottom edge. **The page itself does not
      scroll**; panels scroll inside their own windows.
- [x] 3. The **rail is on the RIGHT** by default, with the workspace to its left.
- [x] 3b. **Overview opened by itself**, because you came in from the dashboard.
...
- [x] 3c. The rail's **bottom** holds, in order: **⊘ Close tabs** in **red**, then
      a divider, then **Settings** as the very last entry. **Overview is not in
      the rail at all**, top or bottom.
- [x] 3d. **"Campaign overview" is a button in the top app bar**, immediately
...
- [x] 3e. **Scheduling is no longer a rail entry.** It is a section at the
...
- [x] 4. Click a rail section — e.g. **NPCs**. It opens **immediately as a
      floating window**. There is no docked panel and no second "pop out" click.
- [x] 5. Click **another** section. It opens as a *second* window, offset from the
      first, and both stay open.
- [x] 6. A rail entry is a **three-way toggle**. With the first window buried
...
- [x] 7. Open sections are marked in the rail (accent bar, bolder text) and the
      marker clears when you close them.
- [x] 8. The window's **✕** closes it too. There is **no ⇤ dock button** any more.

### Windows

- [x] 9. **Drag** by the title bar — smooth, and it can't be dragged fully out of
      reach; some title bar always stays grabbable.
- [x] 10. **Resize from every edge and corner.** The one to watch: dragging the
...
- [x] 11. Panels are fully usable in a window — in **NPCs**, add / edit / delete
      one. Long panels scroll inside the window.
- [x] 12. With three or four windows open, clicking any one **brings it to the
      front**, and they can overlap freely.

### Rail controls

- [x] 13. **Move the rail — now from Settings.** There is **no ⇤/⇥ button in the
...
- [x] 14. **Resize the rail.** Drag its **inner edge** (the one facing the
...
- [x] 15. Move the rail to the other side (**⇤**/**⇥**) and confirm the grab strip
      **moved with it** — it is always on the workspace-facing edge, and dragging
      still grows the rail in the direction you'd expect.
- [x] 16. **Collapse** narrows the rail to single letters; expanding restores it
...
- [x] 17. **Close tabs.** With three windows open the rail's footer shows
...
- [x] 18d. Same for a panel with live data — open **Party** or **HP &
      conditions**, close it, reopen: no spinner, and it is still up to date.
- [x] 19. Console stays clean throughout, including during drags and resizes.

### Persistence

- [x] 20. Open three windows, drag/resize them to distinctive spots, move the rail
...
- [x] 21. Go to the dashboard and back → same arrangement. Open a **different**
      campaign → it has its own independent (default, right-hand) layout; return
      to "Main Test" → yours is intact.

### Playspace / RPG

- [x] 22. Settings → Game mode → **Playspace**. Re-frames **immediately without a
...
- [x] 22b. **Overview is reachable in playspace mode too** — click **Campaign
...
- [x] 23. Close tabs → the playspace has the **whole** area beside the rail.
- [x] 24. Switch to **Full RPG** → same shell, placeholder copy mentions the
      combat tracker. Then switch back to **Note taker**.

### Plan & billing moved (5.2.1b)

- [x] 25. There is **no "Plan & billing" entry in the rail** for the DM.
- [x] 26. **DM** Settings has six blocks in order: **Workspace**, **Campaign
      name**, **Game mode**, **Plan & billing**, **Backup & data**, **Danger
      zone**. Billing shows the same status and actions it did as a tab.

### Player

- [x] 27. As the **player**: the header shows the campaign name with a **Player**
...
- [x] 27b. **Player Settings shows exactly ONE section: Workspace** (sidebar
...
- [x] 27c. The player also gets Overview auto-opening on entry from the dashboard,
      the header button, and the **scheduling section inside Overview** — confirm
      they can see and use scheduling there.
- [x] 28. Player writes still work — **HP & conditions**: change current HP,
      refresh, it stuck.
